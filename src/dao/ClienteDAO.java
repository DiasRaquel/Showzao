...
public class ClienteDAO {
    ...
    public static void editar(Cliente cliente){
        String sql = "UPDATE cidade SET nome = ?, endereco = ?, codCidade = ?  WHERE id = ?";
        PreparedStatement ps = null;
    ...
    public static List<Cliente> getClientes(){
        ...
        String sql = "SELECT p.id, p.nome, p.endereco,c.id, c.nome FROM cliente p " + 
                    "INNER JOIN cidade c ON c.id = p.codCidade ORDER BY p.nome";
        PreparedStatement ps = null;
        try {
            Connection conn = Conexao.getConexao();
            ps = conn.prepareStatement(sql);
